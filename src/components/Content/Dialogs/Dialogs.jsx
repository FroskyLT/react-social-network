...
const Dialogs = (props) => {
    const peopleElements = props.messagesPage.peopleData.map(d => <People id={d.id} name={d.name} imgUrl={d.imgUrl} key={d.id} />);
    const conversationElements = props.messagesPage.conversationData.map(c => c.id
        ? <div className={styles.right} key={c.key}><Message name={c.name} text={c.text} imgUrl={c.imgUrl} /></div>
